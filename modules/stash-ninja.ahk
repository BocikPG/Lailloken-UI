...
Stash_(mode, test := 0)
{
	...
	Loop, % (mode = "delirium" || vars.stash.active = "delirium") ? 3 : 2
	{
		...
		If (league != settings.stash.league) || Blank(timestamp) || Blank(now) || (now >= 61) || (tab = "fragments" && Blank(vars.stash[tab]["tft_Elder Set"].components))
		{
			If !tooltip
...
Stash_PricePicker(cHWND := "")
{
	local
	global vars, settings
	static toggle := 0, available, available0, max_stack, last_currency, sets := ["tft_Unrelenting 5 Way Set","tft_5 Way Set","tft_Sirus Set","tft_Maven's Writ","tft_Shaper Set","tft_Elder Set","tft_Uber Elder Set"]
